...
def download(
    ...
    """
    if manifest is None and not sys.stdin.isatty():
        manifest= sys.stdin.read().strip()
    ...
    if path.is_file():
        ...
        # valid_query = ValidQuery.from_token(manifest)
        # manifest = valid_query.process(store)
        # logger.info(f"Downloading from query token: {manifest}")
